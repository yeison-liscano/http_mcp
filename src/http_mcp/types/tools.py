...
@dataclass
class Tool(Generic[_TArguments_contra, _T, _TOutput_contra]):
    func: Callable[
        [Arguments[_TArguments_contra, _T]],
    ...
    async def invoke(
        ...
        try:
            _args = Arguments(request, validated_args, context)
            if inspect.iscoroutinefunction(self.func):
                return await self.func(_args)

            _func = cast(
                "Callable[[Arguments[_TArguments_contra, _T]], _TOutput_contra]",
                self.func,
            )
